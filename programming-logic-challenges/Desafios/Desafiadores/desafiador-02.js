...
function squareRoot(number){
  ...
  if(typeof number !== "number" || number == 0) return "Número inválido.";
  let sqRoot = Math.sqrt(number);
  let result = Number.isInteger(sqRoot) ? `A raiz quadrada de ${number} é: ${sqRoot}.` : `Não existe raiz exata para o número ${number}.`;
  return result
}
