...
function fantasyGame3(className, level) {
  ...
  let classData = {Mago      : {lifeStart:5,  lifePerLevel:2, weapon: 'Varinha'},
                   Arqueiro  : {lifeStart:10, lifePerLevel:3, weapon: 'Arco Curto'},
                   Guerreiro : {lifeStart:15, lifePerLevel:4, weapon: 'Espada'},
                   Cavaleiro : {lifeStart:20, lifePerLevel:5, weapon: 'Armadura Completa'}};

  let totalLife = classData[className]['lifeStart'] + ((level - 1) * classData[className]['lifePerLevel']);
  return `${className}, nível ${level}: ${totalLife}PV, ${classData[className]['weapon']}.`
}
